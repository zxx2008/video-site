...
def metadata_insert_values(metadata: dict) -> tuple:
    return (
        metadata["duration"],
        metadata["resolution"],
        ...
        metadata["final_display_aspect_ratio"],
        metadata["codec"],
    )


def metadata_from_row(row) -> dict:
    return {
...
def should_refresh_metadata(row) -> bool:
    return (
        not row["sample_aspect_ratio"]
        or not row["display_aspect_ratio"]
        or not row["resolution"]
        or not row["codec"]
        or not row["display_width"]
        or not row["display_height"]
        or not row["final_display_aspect_ratio"]
    )
...
async def upload_video(
    ...
    cursor = await db.execute(
        """INSERT INTO videos (title, filename, storage_path, mime_type, file_size,
           duration, resolution, sample_aspect_ratio, display_aspect_ratio,
           rotation, display_width, display_height, final_display_aspect_ratio, codec)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (title, filename, storage_path, content_type, file_size, *metadata_insert_values(metadata)),
    )
    await db.commit()
    video_id = cursor.lastrowid

    # 生成兼容播放副本（修复旋转、像素宽高比和编码兼容性）
    if needs_playback_normalization(metadata):
...
async def stream_video(video_id: int, request: Request, db=Depends(get_db)):
    cursor = await db.execute(
        """SELECT storage_path, playback_path, mime_type, resolution, codec,
           sample_aspect_ratio, display_aspect_ratio, rotation, display_width,
           display_height, final_display_aspect_ratio FROM videos WHERE id = ?""",
        (video_id,),
    )
    row = await cursor.fetchone()
    if not row:
    ...
    if playback_path and os.path.exists(playback_path):
        ...
        # 老数据补齐元信息
        if should_refresh_metadata(row):
            metadata = probe_video(storage_path)
            await db.execute(
                """UPDATE videos
                   SET resolution = ?, codec = ?, sample_aspect_ratio = ?, display_aspect_ratio = ?,
                       rotation = ?, display_width = ?, display_height = ?, final_display_aspect_ratio = ?
                   WHERE id = ?""",
                (
                    metadata["resolution"],
                    metadata["codec"],
...
async def merge_chunks(
    ...
    cursor = await db.execute(
        """INSERT INTO videos (title, filename, storage_path, mime_type, file_size,
           ...
           rotation, display_width, display_height, final_display_aspect_ratio, codec)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (title, filename, storage_path, mime_type, file_size, *metadata_insert_values(metadata)),
    )
    await db.commit()
    video_id = cursor.lastrowid
